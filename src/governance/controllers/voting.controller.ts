...
export class VotingController {
  ...
  async getVotingHistory(
    ...
    // This would need to be implemented in the voting service
  ...
  async getVotingStats(@Param('proposalId') proposalId: string) {
    ...
    const stats = {
      ...
      weightedVotesFor: votes.filter(v => v.voteType === 'FOR').reduce((sum, v) => sum + v.weightedVote, 0),
      weightedVotesAgainst: votes.filter(v => v.voteType === 'AGAINST').reduce((sum, v) => sum + v.weightedVote, 0),
      weightedVotesAbstain: votes.filter(v => v.voteType === 'ABSTAIN').reduce((sum, v) => sum + v.weightedVote, 0),
    };

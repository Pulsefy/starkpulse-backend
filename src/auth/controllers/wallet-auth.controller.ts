...
export class WalletAuthController {
  ...
  @Get('connect')
  @ApiOperation({ summary: 'Connect Argent X wallet', description: 'Connects an Argent X wallet and returns the address.' })
  @ApiResponse({ status: 200, description: 'Wallet connected', example: { address: '0x123...' } })
  ...
  @Post('nonce')
  @ApiOperation({ summary: 'Get nonce for wallet signature', description: 'Returns a nonce for signing with the wallet.' })
  @ApiBody({ description: 'Wallet address payload', example: { address: '0x123...' } })
  @ApiResponse({ status: 200, description: 'Nonce returned', example: { nonce: 'random-nonce-string' } })

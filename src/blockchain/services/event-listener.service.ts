...
export class EventListenerService implements OnModuleInit, OnModuleDestroy {
  ...
  private async processContractEvents(contract: ContractEntity, latestBlockNumber: number) {
    try {
      ...
      const fromBlock = contract.lastSyncedBlock ? contract.lastSyncedBlock + 1 : Math.max(0, latestBlockNumber - 100);
      ...
      this.logger.debug(`Processing events for contract ${contract.address} from block ${fromBlock} to ${toBlock}`);
      ...
      const filteredEvents = contract.monitoredEvents && contract.monitoredEvents.length > 0
        ? events.filter(event => {
      ...
      await this.contractRepository.update(contract.id, { lastSyncedBlock: toBlock });
    } catch (error) {
      this.logger.error(`Failed to process events for contract ${contract.address}: ${error.message}`);
      throw error;
    }
  }
  ...
  private async saveEvents(contract: ContractEntity, events: StarknetEmittedEvent[]) {
    for (const event of events) {
      try {
        ...
        this.logger.debug(`Saved event ${eventName} for contract ${contract.address} at block ${event.block_number}`);
      } catch (error) {

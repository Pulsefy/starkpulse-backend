...
export class EventProcessorService {
    ...
    try {
      ...
      if (event) {
        ...
        if (retryCount < this.MAX_RETRY_COUNT) {
          ...
          this.logger.debug(`Scheduled retry ${retryCount + 1}/${this.MAX_RETRY_COUNT} for event ${payload.eventId}`);
        } else {
          ...
          this.logger.warn(`Event ${payload.eventId} failed processing after ${this.MAX_RETRY_COUNT} retries`);
        }
      }
    }
  }
  ...
  private async processApprovalEvent(event: EventEntity): Promise<void> {
    ...
    try {
      ...
      if (keys.length >= 3 && data.length >= 1) {
        ...
        this.logger.debug(`Approval: ${owner} approved ${spender} to spend ${amount}`);
  ...
  private async processLiquidityAddedEvent(event: EventEntity): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Error processing LiquidityAdded event: ${error.message}`);
      throw error;
    }
  }
  
  private async processLiquidityRemovedEvent(event: EventEntity): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Error processing LiquidityRemoved event: ${error.message}`);
      throw error;
    }
  }
  ...
  async processUnprocessedEvents(limit: number = 50): Promise<number> {
    try {
      ...
      for (const event of unprocessedEvents) {
        if (!event.contract) {
          this.logger.warn(`Contract not loaded for event ${event.id}, attempting to load`);
  ...
  async cleanupFailedEvents(): Promise<void> {
    try {
      ...
      this.logger.debug(`Found ${failedEvents.length} failed events to clean up`);
